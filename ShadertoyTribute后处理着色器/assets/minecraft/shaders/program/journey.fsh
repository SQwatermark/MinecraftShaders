...
//==========================================================================================
// 技术难点
// 整个着色器是抄来的而且没啥bug，但接入原版mc的后处理着色器还是有不少问题
// 问题1：无法获得持续的时间，后处理着色器只能接受从0到1循环的时间。
// 解决1：我用帧缓冲解决了计时问题，理论上计时器可持续运行194.18天。
// 问题2：原着色器沙海表面的质感非常舒服而直接放进mc里就变得全是噪点。这是缺少mipmap的缘故。
// 解决2：因为着色器里只用了一种Lod，我直接用PS把iChannel1给降低了分辨率，现在效果接近原版效果了
// 问题3：玩家移动位置和旋转视角没有作用。
//==========================================================================================

//==========================================================================================
// 以下为 https://www.shadertoy.com/view/ldlcRf 修改版本
//==========================================================================================
...
void drawDebugInfo() {
	// 绘制调试字符
    // clearTextBuffer();
    // c('i'); c('T'); c('i'); c('m'); c('e'); c(':'); c(' '); floatToDigits(iTime);
    // printTextAt(1.0, 1.0);

	clearTextBuffer();
    c('R'); c(':'); c(' '); floatToDigits(glowInfo.r);
    printTextAt(1.0, 2.0);

	clearTextBuffer();
    c('G'); c(':'); c(' '); floatToDigits(glowInfo.g);
    printTextAt(1.0, 3.0);

	clearTextBuffer();
    c('B'); c(':'); c(' '); floatToDigits(glowInfo.b);
    printTextAt(1.0, 4.0);

	clearTextBuffer();
    c('A'); c(':'); c(' '); floatToDigits(glowInfo.a);
    printTextAt(1.0, 5.0);
}
#endif
...
void main()
{
	...
	glowInfo = texture(GlowingSampler, vec2(0.5, 0.5));
	...
	vec3 screenRay		= vec3(texCoord, 1.0);
